...
Before designing an API, consider:
- API consumer requirements and client types
- Appropriate style selection (REST, GraphQL, or tRPC)
- Consistent response formatting (RFC 7807 for errors)
- Versioning strategy (URI path vs header)
...
- Using verbs in endpoint naming (`/getUsers` → `/users`)
- Inconsistent response structures across endpoints
- Exposing system internals in error messages
- Omitting rate-limiting protections on public endpoints
- Automatically choosing REST when GraphQL or tRPC fits better
